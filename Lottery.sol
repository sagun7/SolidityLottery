...
contract Lottery {
    ...
    address payable[] public participants; /* "payable" was used because once we find the winner, we need to 
    ...
    {
        require(msg.value==1 ether); /* if this condition is satisfied, the below line will be executed else not..
        meaning: if the user is not paying 1 ether, participant's address will not be sent to array of participants  */
        participants.push(payable(msg.sender)); /* this line will push the address of sender(participant) into array
    ...
    {
        require(msg.sender==manager); /* if the sender address(current user's address.. when we choose from the list of account) 
    ...
    */
    function random() public view returns(uint) // this function will give random value
    {
    ...
    {
        require(msg.sender==manager); // manager is doing this things
        require(participants.length >= 3);
        uint r=random();
        address payable winner;
        uint index = r % participants.length; // index value will always be less than players.length..
